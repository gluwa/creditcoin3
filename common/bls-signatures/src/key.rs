...
pub trait Serialize: Debug + Sized {
    ...
    fn as_bytes(&self) -> Vec<u8> {
        let res = Vec::with_capacity(8 * 4);
        let mut writer = VecWriter(res);
        self.write_bytes(&mut writer).expect("preallocated");
        writer.0
    }
}
...
impl Serialize for PrivateKey {
    fn write_bytes(&self, dest: &mut impl Write) -> Result<(), Error> {
        for digit in &self.0.to_le_bits().data {
            match dest.write_all(&digit.to_le_bytes()) {
                Ok(()) => {}
                Err(_) => return Err(Error::SizeMismatch),
            }
        }
...
impl Serialize for PublicKey {
    fn write_bytes(&self, dest: &mut impl Write) -> Result<(), Error> {
        ...
        dest.borrow_mut().write_all(tmp.as_ref())?;
