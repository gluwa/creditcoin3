...
TRACE_SIZE=$(echo $(stat -c "%s" $TRACE_FILE) | tr -d '"')
echo "trace file size: $TRACE_SIZE"
CHAIN_LEN=$(jq --arg PROGRAM_INPUT_FILE "$PROGRAM_INPUT_FILE" '.blocks | length' $PROGRAM_INPUT_FILE)
echo "chain length from the input file: $CHAIN_LEN"
PUBLIC_INPUT_SIZE=$(echo $(stat -c "%s" $PUBLIC_INPUT) | tr -d '"')
PRIVATE_INPUT_SIZE=$(echo $(stat -c "%s" $PRIVATE_INPUT) | tr -d '"')

TRACE_SIZE_DIV3=$(($TRACE_SIZE / 3))
LOG_TRACE_SIZE_DIV3=$(echo $TRACE_SIZE_DIV3 | awk '{print log($1)/log(2)}')
FRI_STEPS_SUM=$(jq --arg AIR_PARAMS "$AIR_PARAMS" '.stark.fri.fri_step_list | add' $AIR_PARAMS)

LOG_LAST_LAYER_DEGREE_BOUND=$(echo $(($LOG_TRACE_SIZE_DIV3 - $FRI_STEPS_SUM)) | tr -d '"')
...
jq --arg LAST_LAYER_DEGREE_BOUND "$LAST_LAYER_DEGREE_BOUND" --arg AIR_PARAMS "$AIR_PARAMS" \
