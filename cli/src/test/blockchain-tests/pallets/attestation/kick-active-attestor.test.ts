...
describe('kickActiveAttestor', (): void => {
    let api: ApiPromise;
    ...
    beforeAll(async () => {
        ...
        await api.tx.attestation
            .authorizeAttestor(chain_Anvil2_Key, attestorAccount.address)
    ...
    it('fee is min 0.01 CTC', async (): Promise<void> => {
        ...
        return new Promise((resolve, reject): void => {
            ...
            const kick = api.tx.sudo
                .sudo(api.tx.attestation.kickActiveAttestor(chain_Anvil2_Key, attestorAccount.address, true))
                .signAndSend(root, { nonce }, async ({ dispatchError, events, status }) => {
                    await extractFee(resolve, reject, kick, api, dispatchError, events, status);
                })
