...
describe('removeAuthorizedAttestor', (): void => {
    let api: ApiPromise;
    ...
    it('fee is min 0.01 CTC', async (): Promise<void> => {
        ...
        return new Promise((resolve, reject): void => {
            ...
            const remove = api.tx.sudo
                .sudo(api.tx.attestation.removeAuthorizedAttestor(chain_Anvil2_Key, attestorAccount.address))
                .signAndSend(root, { nonce }, async ({ dispatchError, events, status }) => {
                    await extractFee(resolve, reject, remove, api, dispatchError, events, status);
                })
