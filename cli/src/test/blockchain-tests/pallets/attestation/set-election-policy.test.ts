...
describe('setElectionPolicy', (): void => {
    let api: ApiPromise;
    ...
    it('fee is min 0.01 CTC', async (): Promise<void> => {
        ...
        return new Promise((resolve, reject): void => {
            ...
            const policy = api.tx.sudo
                .sudo(api.tx.attestation.setElectionPolicy(chain_Anvil2_Key, 'OpenToAny'))
                .signAndSend(root, { nonce }, async ({ dispatchError, events, status }) => {
                    await extractFee(resolve, reject, policy, api, dispatchError, events, status);
                })
