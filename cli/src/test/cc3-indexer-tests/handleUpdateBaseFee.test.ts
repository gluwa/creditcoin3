...
describe('handleUpdateBaseFee()', () => {
    let api: ApiPromise;
    let alith: any;
    let initialBaseFee = 0n;
    let contractAddress = '';
    ...
    beforeAll(async () => {
        ...
        for (const node of response.data.provers.nodes) {
            if (node.owner === alith.address) {
                initialBaseFee = BigInt(node.baseFee);
                contractAddress = node.contractAddress;
                // NOTE: will operate on contract for last prover deployed for this source chain
            }
        }
        expect(initialBaseFee).toBeGreaterThan(0);
        expect(contractAddress.startsWith('0x')).toEqual(true);
    }, 30_000);

    describe('when prover updates their base fee', () => {
        let newBaseFee: bigint;

        beforeAll(async () => {
            newBaseFee = initialBaseFee + 444n;

            const contract = new ethers.Contract(contractAddress, contractABI, provider);

            await (await (contract.connect(alith) as any).updateBaseFee(newBaseFee)).wait();
        ...
        it('graphQL returns updated Prover entities', async () => {
            ...
            for (const node of response.data.provers.nodes) {
                ...
                // fee was updated
                expect(BigInt(node.baseFee)).toEqual(newBaseFee);
            }
        });
    });
});
