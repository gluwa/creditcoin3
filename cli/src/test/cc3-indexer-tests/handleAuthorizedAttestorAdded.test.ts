...
describe('handleAuthorizedAttestorAdded()', () => {
    let api: ApiPromise;
    let bob: KeyringPair;
    ...
    describe('when attestor is added to authorized set', () => {
        beforeAll(async () => {
        ...
        it('graphQL returns AuthorizedAttestorAdded entity', async () => {
            const response = await graphQLQuery(
                `query { authorizedAttestorAddeds(orderBy: BLOCK_NUMBER_ASC, last: 1, filter: {attestorId: {equalTo: "${attestor.address}"}}) { nodes { id, blockNumber, date, chainKey, attestorId }}}`,
            );
            expect(response.data.authorizedAttestorAddeds.nodes).toBeTruthy();
            expect(response.data.authorizedAttestorAddeds.nodes.length).toEqual(1);

            for (const node of response.data.authorizedAttestorAddeds.nodes) {
        ...
        it('graphQL returns AuthorizedAttestors entity', async () => {
            const response = await graphQLQuery(
                `query { authorizedAttestors(orderBy: CHAIN_KEY_ASC, last: 100, filter: {attestorId: {equalTo: "${attestor.address}"}}) { nodes { id, chainKey }}}`,
            );
            expect(response.data.authorizedAttestors.nodes.length).toEqual(1);
            for (const node of response.data.authorizedAttestors.nodes) {
                expect(node.id).toEqual(`${chain_Anvil2_Key.toString()}-${attestor.address}`);
                expect(Number(node.chainKey)).toEqual(chain_Anvil2_Key);
            }
        });
    });
