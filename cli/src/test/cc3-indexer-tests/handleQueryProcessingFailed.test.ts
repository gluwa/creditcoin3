...
describe('handleQueryProcessingFailed()', () => {
    let api: ApiPromise;
    let alith: any;
    let contract: any;
    let queryId = '';
    ...
    beforeAll(async () => {
        ...
        contract = await factory.deploy(
            ...
            1000n,
            sampleQuery.chainId,
            'Prover-for-handleQueryProcessingFailed',
        ...
        // submit query as Submitted state so we can mark it as processing failed
        const receipt = await (
    ...
    describe('when a query is marked as processing failed', () => {
        ...
        beforeAll(async () => {
            ...
            await (await contract.markProcessingFailed(queryId, failureReason)).wait();

            // Verify the query state changed on-chain
            queryDetailsOnChain = await contract.queries(queryId);
            expect(queryDetailsOnChain.state).toEqual(4n); // QueryState.QueryProcessingFailed
            expect(queryDetailsOnChain.escrowedAmount).toEqual(0n); // Refunded

            await forElapsedBlocks(api, { minBlocks: 3 });
        }, 45_000);

        it('ChainQueries entity is correctly updated with QueryProcessingFailed state and failure reason', async () => {
            ...
            // Verify state is updated to QueryProcessingFailed
            expect(node.state).toEqual('QueryProcessingFailed');
