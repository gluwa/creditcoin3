...
describe('handleCheckpointIntervalChanged()', () => {
    ...
    describe('when attestations per checkpoint is changed', () => {
        ...
        it('graphQL returns known CheckpointIntervalChanged entity', async () => {
            const response = await graphQLQuery(
                `query { checkpointIntervalChangeds(orderBy: BLOCK_NUMBER_ASC, last: 1) { nodes { id, blockNumber, date, chainKey, interval }}}`,
            );
            expect(response.data.checkpointIntervalChangeds.nodes).toBeTruthy();
            expect(response.data.checkpointIntervalChangeds.nodes.length).toEqual(1);
        ...
        it('graphQL returns updated AttestationChainData entity', async () => {
            ...
            for (const node of response.data.attestationChainData.nodes) {
                expect(node.checkpointInterval).toEqual(newInterval);
            }
        });
    });
});
