...
describe('integration test: validator manual setup', () => {
    it.each([
        ...
        async (text, ecdsa) => {
            ...
            const stashStatus = await getValidatorStatus(
                stashAddress,
                aliceApi
            );
            ...
            const stashBondedBalance = (
                await getBalance(stashAddress, aliceApi)
            ).bonded;
            ...
            const nodeHasKeys = (
                await bobApi.rpc.author.hasSessionKeys(newKeys)
            ).isTrue;
            ...
            const balanceBeforeRewards = await getBalance(
                stashAddress,
                aliceApi
            );
            ...
            // wait 5 seconds for nodes to sync
            await new Promise((resolve) => setTimeout(resolve, 5000));
            const balanceAfterRewards = await getBalance(
                stashAddress,
                aliceApi
            );
            ...
            // wait 5 seconds for nodes to sync
            await new Promise((resolve) => setTimeout(resolve, 5000));
            const balanceAfterUnbonding = await getBalance(
                stashAddress,
                aliceApi
            );
            ...
            expect(balanceAfterWithdraw.transferable.gte(stashAmount)).toBe(
                true
            );
        },
        2000000
