...
//! works against a local Anvil node and a fixture libp2p mesh. Gated behind the
//! `integration-tests` feature like proof-gen-api-server's E2E tests.
//!
...
async fn message_published_to_message_delivered_round_trip() {
    // Steps:
    // 1. Boot anvil via alloy-node-bindings; deploy dummy Outbox + Inbox.
