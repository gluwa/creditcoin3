...
export async function handleSupportedChainRegistered(event: SubstrateEvent): Promise<void> {
    ...
    const newChain = AttestationChainData.create({
        ...
        maxSetSize: 100,
        targetSampleSize: 3,
        minBondRequirement: BigInt(0), // matches runtime DefaultMinBondRequirement
...
export async function handleEventCheckpointReached(event: SubstrateEvent): Promise<void> {
    ...
    // Drop prior indexer rows at this checkpoint height only (digest overwrite / re-import).
    // We intentionally do not call `remove_checkpoints_above_height(chainKey, checkpoint.blockNumber - 1)` here:
    // that would delete every checkpoint >= this height, which is wrong for `forward_patch_checkpoints`
    // when `wipe_suffix` is false — the runtime keeps checkpoints strictly above the patch batch.
    // Suffix removal when `wipe_suffix` is true is handled in `handleEventForwardCheckpointPatchApplied`.
    await remove_checkpoints_at_checkpoint_height(chainKeyNumber, checkpoint.blockNumber);
...
export async function handleEventForwardCheckpointPatchApplied(event: SubstrateEvent): Promise<void> {
    logger.info(`ForwardCheckpointPatchApplied at block ${event.block.block.header.number.toString()}`);

    const {
        event: {
            data: [chainKey, wipedSuffix, tipBlockNumber],
        },
    } = event;

    const chainKeyNumber = BigInt(chainKey.toString());
    const wipedSuffixBool = codecToBool(wipedSuffix);
    const tipBlock = BigInt(tipBlockNumber.toString());

    // Purge mirrors `purge_attestations_for_forward_patch`: on-chain clears the whole attestation prefix map.
    // Using `remove_attestations_above_height(..., 0)` removes every indexed row with headerNumber > 0.
    // (A threshold of `checkpoint.blockNumber - 1` per `CheckpointReached` would leave stale rows below the patch.)
    await remove_attestations_above_height(chainKeyNumber, BigInt(0));

    // When suffix was wiped, chain removed checkpoints strictly above the batch tip.
    if (wipedSuffixBool) {
        await remove_checkpoints_above_height(chainKeyNumber, tipBlock);
    }

    const chainData = await getChainData(chainKeyNumber);
    if (chainData) {
        chainData.lastCheckpointHeaderNumber = tipBlock;
        chainData.lastAttestedHeaderNumber = BigInt(0);
        chainData.lastAttestedDigest = '';
        await chainData.save();
    } else {
        logger.warn(
            `AttestationChainData not found for chainKey=${chainKeyNumber.toString()} during ForwardCheckpointPatchApplied`,
        );
    }
}
...
function codecToBool(value: unknown): boolean {
    if (typeof value === 'boolean') {
        return value;
    }
    if (value && typeof value === 'object') {
        const v = value as { isTrue?: boolean; isFalse?: boolean };
        if (typeof v.isTrue === 'boolean') {
            return v.isTrue;
        }
        if (typeof v.isFalse === 'boolean') {
            return !v.isFalse;
        }
        const jsonish = value as { toJSON?: () => unknown };
        if (typeof jsonish.toJSON === 'function') {
            const j = jsonish.toJSON();
            if (typeof j === 'boolean') {
                return j;
            }
        }
...
async function remove_checkpoints_at_checkpoint_height(chainKey: bigint, checkpointHeight: bigint): Promise<void> {
    ...
    while (true) {
        const page = await Checkpoints.getByFields(
            [
                ['chainKey', '=', chainKey],
                ['blockNumber', '=', checkpointHeight],
            ],
            {
                limit: PAGE_SIZE,
                offset,
