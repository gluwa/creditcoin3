...
OUTPUT="./pallets/${PALLET//_/-}/src/weights1.rs"
...
if [[ $BENCH -eq 0 ]]
then
    ./target/release/creditcoin3-node benchmark pallet --chain dev --steps="$STEPS" --repeat="$REPEAT" --pallet "pallet_$PALLET" --extrinsic='*' --execution wasm --wasm-execution=compiled --heap-pages=10000 --output "$OUTPUT"
