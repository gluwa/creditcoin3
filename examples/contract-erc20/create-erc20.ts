...
async function init() {
  ...
  const aliceEvmAccount = `0x${crypto
    .blake2AsHex(crypto.decodeAddress(alice.address), 256)
    .substring(26)}`;

  console.log(`Alice EVM Account: ${aliceEvmAccount}`);
  const evmData = (await api.query.evm.accounts(aliceEvmAccount)) as any;
  console.log(
    `Alice EVM Account (nonce: ${
      evmData.nonce
    }) balance: ${evmData.balance.toHex()}`,
  );
    ...
    async (resolve, reject) => {
      const unsub = await transaction.signAndSend(alice, (result) => {
        ...
        if (result.status.isInBlock) {
          ...
          console.log(
            `Contract finalized at blockHash ${result.status.asFinalized}`,
          );
...
async function step2(
  ...
  const accountCode = (
    await api.query.evm.accountCodes(contractAddress)
  ).toString();
  console.log(
    `Contract account code: ${accountCode.substring(
      0,
      16,
    )}...${accountCode.substring(accountCode.length - 16)}`,
  );
  ...
  const accountStorage = (
    await api.query.evm.accountStorages(contractAddress, storageKey)
  ).toString();
...
async function step3(
  ...
  const bobEvmAccount = `0x${crypto
    .blake2AsHex(crypto.decodeAddress(bob.address), 256)
    .substring(26)}`;
  console.log(
  ...
  const inputCode = `0x${transferFnCode}${bobEvmAccount.substring(
    2,
  )}${tokensToTransfer}`;
  ...
  const data = new Promise<{ block: string; address: string }>(
    async (resolve, reject) => {
      const unsub = await transaction.signAndSend(alice, (result) => {
...
async function step4(
  ...
  console.log(
    `Bob Substrate Account (nonce: ${nonce}) balance, free: ${balance.free}`,
  );
  const bobEvmAccount = `0x${crypto
    .blake2AsHex(crypto.decodeAddress(bob.address), 256)
    .substring(26)}`;
