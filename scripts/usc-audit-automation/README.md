...
A Deno-based TypeScript tool that runs attestation sanity checks on USC (Creditcoin3)
and reports to Slack or stdout. Matches the style of the [traffic-simulator](../traffic-simulator).

All configuration is loaded from a single JSON file. For CI, env overrides: `USC_SLACK_WEBHOOK_URL`, `USC_SLACK_ALERT_GROUP`, `SEPOLIA_RPC_URL`, `BSC_RPC_URL`.
...
Create a JSON config file. All settings live here—no `.env` or environment variables.
...
- **ethRpc**: Array of `{ chainId, chainKey?, url }`; `chainKey` optional (discovered from USC if omitted)
- **slackWebhookUrl**, **slackAlertGroup**: Optional; required only when not using `--no-slack`

**Env overrides (CI)**: `SEPOLIA_RPC_URL` overrides url for chainId 11155111; `BSC_RPC_URL` for chainId 97.

Relative config paths (e.g. `config-devnet.json`) are resolved from the script directory, so it works regardless of current working directory.
