...
export function loadConfig(): SimulatorConfig {
  ...
  const config: SimulatorConfig = {
    ...
    // Proof API
    proofApiUrl: getString(
      args,
      "api-url",
      "PROOF_API_URL",
      DEFAULTS.proofApiUrl,
    ),

    // Simulation parameters
    maxQueueSize: getNumber(
      args,
      "max-queue-size",
      "MAX_QUEUE_SIZE",
...
function validateConfig(config: SimulatorConfig): void {
  // Validate URLs
  if (
    !config.sourceRpcUrl.startsWith("ws://") &&
    !config.sourceRpcUrl.startsWith("wss://")
  ) {
    throw new Error("Source RPC URL must be a WebSocket URL (ws:// or wss://)");
  }

  if (
    !config.cc3WsUrl.startsWith("ws://") &&
    !config.cc3WsUrl.startsWith("wss://")
  ) {
  ...
  if (
    !config.cc3PrivateKey.startsWith("0x") || config.cc3PrivateKey.length !== 66
  ) {
    throw new Error(
      "Private key must be a 32-byte hex string starting with 0x",
    );
  }
  ...
  if (
    !Number.isInteger(config.singleEveryBlocks) || config.singleEveryBlocks < 1
  ) {
...
export function logConfig(config: SimulatorConfig): void {
  const masked = {
    ...config,
    cc3PrivateKey: config.cc3PrivateKey.slice(0, 6) + "..." +
      config.cc3PrivateKey.slice(-4),
  };
