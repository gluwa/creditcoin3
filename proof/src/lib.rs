...
use ethereum::{EIP1559Transaction, EIP2930Transaction, LegacyTransaction, TransactionSignature};
...
pub fn cairo_verify_claim<H, Address, A>(
    url: &str,
    claim: Claim<Address>,
    attestation_fragment: &Attestation<H, A>,
) -> anyhow::Result<()> {
    ...
    let attestation_chain_slice = attestation_fragment.attestation_slice_for(claim_block_number, None)
        .ok_or(anyhow!("can't create attestation checkpoint slice for {} on this attestation chain ({:?}, {:?})",
            claim_block_number,
            attestation_fragment.tail().map(|block| block.n),
            attestation_fragment.head().map(|block| block.n)))?;
