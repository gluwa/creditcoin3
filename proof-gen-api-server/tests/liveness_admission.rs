...
/// both so concurrency is observable. Since the prover confirms heights against the attested
/// set (#1365) the request path never reads the source tip, so the tip counters double as a
/// regression check that it stays that way; the block fill is the slow upstream call now.
struct CountingEth {
...
impl CountingEth {
    ...
    /// A provider whose first block fill takes `delay`: the slow upstream call a request can
    /// actually be waiting on.
    fn with_slow_first_fetch(mut self, delay: Duration) -> Self {
...
async fn a_burst_of_identical_cold_requests_is_capped_and_fills_the_block_once() {
    ...
    assert_eq!(
        provider.peak_tip.load(Ordering::SeqCst),
        0,
        "the request path never reads the source tip: maturity comes from the attestation cache"
    );
    assert_eq!(
...
async fn a_request_past_the_deadline_is_cut_off_with_504() {
    let provider = Arc::new(
        CountingEth::new(Duration::from_millis(100)).with_slow_first_fetch(Duration::from_secs(2)),
    );
    let admission = AdmissionConfig {
