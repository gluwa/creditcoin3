...
#[tokio::test]
async fn health_reports_serving_ability_not_primary_reachability() {
    ...
    // The primary cannot serve the tip, the fallback can: proofs can still be produced, so
    // the source is healthy and no repair is started.
    assert!(provider.is_healthy().await.unwrap());
    assert_eq!(
        provider.generation(),
        0,
