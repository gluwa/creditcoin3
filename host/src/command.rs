...
pub mod tests {
    ...
    fn verifying_stark_proof_with_bogus_public_memory_section_should_error() {
        ...
        // the proof itself which is none of the existing hashes defined in StarkProgramMetadataStorage
        assert!(result.is_err());
        assert_eq!(
            result.err(),
            Some(
                "Failed to authenticate STARK program: AuthenticationFailure(1832824068575451560)"
                    .into()
            )
        );
    }
    ...
    fn verifying_correct_stark_proof_when_program_metadata_config_is_different_should_error() {
        ...
        let metadata = vec![(1, StarkProgramMetadataStorage::AUTH_HASH_V2_DEV)];
        let last_version = StarkProgramMetadataStorage::V2_DEV;
