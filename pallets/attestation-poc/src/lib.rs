...
pub mod pallet {
    ...
    pub type MaxAttestors<T: Config> = StorageValue<_, u32, ValueQuery, MaxAttestorDefault<T>>;

    #[pallet::storage]
    #[pallet::getter(fn max_invulnerables)]
    pub type MaxInvulnerables<T: Config> =
        StorageValue<_, u32, ValueQuery, MaxInvulnerablesDefault<T>>;

    #[pallet::type_value]
    pub fn MaxAttestorDefault<T: Config>() -> u32 {
    ...
    #[pallet::type_value]
    pub fn MaxInvulnerablesDefault<T: Config>() -> u32 {
        100
    }
    ...
    pub enum Error<T> {
        ...
        /// The call to register_invulnerable_failed, the address supplied is not currently registered as an attestor
        AddressNotAttestor,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_invulnerable(origin: OriginFor<T>) -> DispatchResult {
            ...
            Self::try_insert_invulnerable(&who)
        }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn try_insert_attestor(address: &T::AccountId) -> DispatchResult {
            ensure!(
        ...
        fn try_insert_invulnerable(address: &T::AccountId) -> DispatchResult {
            ensure!(
...
mod tests {
    ...
    fn register_attestor_should_fail_when_list_is_full() {
        ExtBuilder.build_and_execute(|| {
            ...
            assert_err!(
                Attestation::register_attestor(RuntimeOrigin::signed(ATTESTOR_2)),
    ...
    fn set_max_attestors_should_error_with_non_root_origin() {
        ExtBuilder.build_and_execute(|| {
            assert_err!(
                Attestation::set_max_attestors(RuntimeOrigin::signed(ATTESTOR_1), 200),
                BadOrigin
            )
        })
    }
    ...
    fn set_max_invulnerables_should_error_with_non_root_origin() {
        ExtBuilder.build_and_execute(|| {
            assert_err!(
                Attestation::set_max_invulnerables(RuntimeOrigin::signed(ATTESTOR_1), 200),
                BadOrigin
            )
        })
    }
