use crate::{
    attestation_checks::{self, UniversalSmartContractProvider},
...
use crate::attestation_checks::MockEthereumProvider;
...
async fn get_attestor_best_block_height_fetches_best_block_correctly() -> Result<()> {
    ...
    let signed_attestation =
        attestation_checks::get_attestor_latest_attestation_data(&client, target).await?;
...
async fn get_attestor_best_block_height_returns_an_error_when_no_best_block_found() -> Result<()> {
    ...
    // Call mock get_attestor_best_block_height
    let result = attestation_checks::get_attestor_latest_attestation_data(&client, target).await;
...
async fn get_ethereum_current_block_number_correctly_gets_block_number() -> Result<()> {
    let mut mock = attestation_checks::MockEthereumProvider::new(); // generated by mock call
    ...
    let eth_block_number = attestation_checks::get_ethereum_current_block_number(&mock).await?;
...
async fn get_ethereum_current_block_number_correctly_handles_an_error() -> Result<()> {
    let mut mock = attestation_checks::MockEthereumProvider::new();

    mock.expect_fetch_block_number()
        .returning(|| (async { Err(anyhow::anyhow!("RPC error: connection dropped")) }).boxed());

    let result = attestation_checks::get_ethereum_current_block_number(&mock).await;
...
fn create_message_returns_chain_status_and_group_notification_when_slack_group_is_some_and_block_diff_check_fails(
    ...
    let ethereum_current_block_number = 200_u64;
    ...
    let expected_result = (
        ...
        Some(json!({
            ...
            "text": "<!subteam^TEST_GROUP> The following issues were detected:\nCurrent block difference exceeds threshold!"
        })),
    );
...
fn create_message_returns_chain_status_and_no_group_notification_when_slack_group_is_none_and_all_checks_fail(
    ...
    let ethereum_current_block_number = 200_u64;
    let etheruem_current_block_calculated_merkle_root = "0xsomecalculatedmerkle".to_string();
    let fetched_ethereum_block_number_by_hash = Some(U64::from(100u64));
    ...
    let result = create_json_message(
        ...
        last_signed_attestation,
        ethereum_current_block_number,
        etheruem_current_block_calculated_merkle_root,
        fetched_ethereum_block_number_by_hash,
        slack_alert_group,
    );
...
async fn test_calculate_merkle_root_returns_expected_bytes() {
    // Prepare a dummy OrderedBlock and expected root
    let chain_id = 1u64;
    let number = 123u64;
    let hash = &[0u8; 32];
    ...
    let dummy_block = Arc::new(dummy_block); // Wrap in Arc

    // Set up the mock client
    let mut mock_client = MockEthereumProvider::new();
    let block_clone = Arc::clone(&dummy_block);
    mock_client
    ...
    // Call the function
    let result = attestation_checks::calculate_merkle_root(&mock_client, 123).await;
